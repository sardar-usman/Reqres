...
function getFixtureInfo(fixtureInput) {
  if (typeof fixtureInput === 'string') {
    return {
      ...
      encoding: '',
      mimeType: '',
      fileName: path__default['default'].basename(fixtureInput)
    };
  }
  ...
  return {
    ...
    fileName: fixtureInput.fileName || path__default['default'].basename(fixtureInput.filePath),
...
function getFileBlobAsync({
  ...
  return getBlob(fileContent, mimeType).then(blob => {
    ...
    if (path.extname(fileName).slice(1) === FILE_EXTENSION.JSON) {
